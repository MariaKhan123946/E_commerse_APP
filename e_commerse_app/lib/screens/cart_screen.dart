...
class _CartScreenState extends State<CartScreen> {
    ...
    try {
      ...
      List<Map<String, dynamic>> items = snapshot.docs.map((doc) {
        ...
        return {
          ...
          'price': data.containsKey('price') ? data['price'].toString() : '0.00',
  ...
  double getTotalPrice() {
    ...
    for (var item in cartItems) {
      double price = double.tryParse(item['price'].replaceAll('\$', '')) ?? 0.00;
  ...
  Widget build(BuildContext context) {
    double total = getTotalPrice();
    double shipping = 0.0;  // You can set shipping cost dynamically if required
